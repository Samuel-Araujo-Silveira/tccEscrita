...
\begin{itemize}
	...
	\item \textbf{MLL}: esse componente é responsável pela aprendizagem de máquina, ou seja, ele irá executar modelos treinados e manipular as saídas por meio dos parâmetros definidos;
...
Anteriormamente à IA, o ser humano era o único responsável pela aprendizagem e desenvolvimento de soluções para problemas complexos. No entanto, a inteligência artificial surge como uma alternativa para essa realidade \cite{SICHMAN}.

\subsection{Aprendizagem de Máquina}

A Inteligência Artificial (IA) possui uma ramificação nomeada de \sigla{AM}{Aprendizagem de Máquina} cujo foco é utilizar grandes quantidades de dados para criar e treinar modelos. Esses modelos, por sua vez, simulam o comportamento humano de resolução de problemas complexos que necessitam de interpretações para serem solucionados, ou seja, um algoritmo convencional não tem como resolvê-los \cite{SMOLA}. 

De acordo com \citeonline{DEISENROTH}, o cerne da aprendizagem de máquina gira em torno de dados, modelos e aprendizagem. Os dados possuem uma característica basilar para o AM, já que são eles os responsáveis pela etapa crucial de treinamento. Os modelos, por sua vez, são responsáveis por lidar com situações não apresentadas no \textit{dataset}, ou seja, aqui há a etapa de Aprendizagem. 

Além do mais, segundo \citeonline{agrawal2020}, o AM possui dois tipos de variáveis vitais para a aprendizagem do modelo: os hiperparâmetros e os parâmetros. O primeiro é de caráter manual, pois é necessário determiná-los previamente em relação ao treinamento. O segundo é volátil, uma vez que o próprio modelo é incumbido da responsabilidade de alterá-los à medida que novas informações vão sendo alimentadas.

\subsection{Redes Neurais Artificiais}
De acordo com \citeonline{HAYKIN}, o \sigla{RNA}{Redes Neurais Artificiais} são estudadas e aplicadas pelo fato de que o cérebro humano é capaz de processar informações de forma muito mais eficiente do que um computador por causa das propriedades de complexidade, não-linearidade e paralelismo presentes no cérebro humano. Portanto, um sistema de aprendizagem possui muito a ganhar caso consiga reproduzir tal estrutura. 
...
Após o desenvolvimento do modelo de aprendizagem de máquina, torna-se vital averiguar sua eficiência. Para isso, há três importantes passos \cite{kashyap2024precision}: 

\begin{itemize}
	\item \textbf{Precisão}: Mede a acurácia das previsões positivas. Ou seja, cada região que o modelo classificar como contendo um objeto de interesse será analisada para verificar se há predição condiz com a verdade ou não;
	
	\item \textbf{\textit{Recall} (Sensitividade)}: Mede a capacidade do modelo de identificar todos os objetos de interesse definidos no pré-processamento;
	
	\item \textbf{\textit{F1Score}}: Balanceia as duas métrica em um único valor.
\end{itemize}